...
for (int i = 0; i < 8; i++){ //iterating through rows
  for (int j = 0; j < 8; j++){ //iterating through columns
    //Black Piece Cases
    if (board[i][j] == 1 && j < 6 && ((board[i+1][j+1] == 2) && (board[i+2][j+2] == 0))){
      return true;
    }
    else if (board[i][j] == 1 && j > 1 && ((board[i+1][j-1] == 2) && (board[i-2][j-2] == 0))){
      return true;
    }
    
    //White Piece Cases
    else if (board[i][j] == 2 && j < 6 && ((board[i-1][j+1] == 2) && (board[i-2][j+2] == 0))){
      return true;
    }
    else if  (board[i][j] == 2 && j > 1 && ((board[i-1][j-1] == 2) && (board[i-2][j-2] == 0))){
      return true;
    }
    else if (board[i][j] == 3){
      //black king check
...
void makeMove(bool blacksTurn){
  ...
  cout << endl << canCapture(); // DEBUGGINGPURPOSE
  do{
