...
bool legalMove(bool blacksTurn, int currentRow, int currentCol, int toRow, int toCol, bool update);
...
void allLegalMoves(bool blacksTurn) {
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if (board[i][j] != 0) {
        if (legalMove(blacksTurn,i, j, i+1, j+1,false)) {
        ...
        if (legalMove(blacksTurn,i, j, i+2, j+2,false)) {
...
bool canCapture(bool blacksTurn){
  for (int i = 0; i < 8; i++){ //iterating through rows
    for (int j = 0; j < 8; j++){ //iterating through columns
        ...
        else if (legalMove(blacksTurn,i,j,i-2,j-2,false)){
          return true;
        }
        //Extra King Checks
        else if (legalMove(blacksTurn,i,j,i-2,j+2,false)){
...
void makeMove(bool blacksTurn){
  ...
  if (blacksTurn){
    ...
    do {
      ...
      toCol = to.at(0) - 97;
      toRow = to.at(1) - '0' - 1;
    } while (!legalMove(blacksTurn,currentRow,currentCol,toRow,toCol,true));
  }
  else{
    legalMove(blacksTurn,AIMove[2]/1000,AIMove[2]%1000/100,AIMove[2]%100/10,AIMove[2]%10,true); //** RICHARD'S AI CODE
  }
